...
if(window.XMLHttpRequest)
	...
	//open XML file	
  	requestcategory.open("GET", "xml/categoryTwo.xml", true);
		//call on success
	  requestcategory.onreadystatechange = function() {
		...
		if (requestcategory.readyState == 4 && requestcategory.status == 200) {
		   ...
		   question();//call of function for question
		}
		};
	//send http request	
  	requestcategory.send();
		...
		//Question function
		  function question() {	
			  ...
			  option+="</div>"; 	  
			   document.getElementById("option").innerHTML=option;//insert the options
		  }
